...
namespace Hoangldp.Web.Data
{
    ...
    public class Repository<TEntity> : BaseRepository<TEntity>, IDisposable where TEntity : class, IEntity, new()
    {
        ...
        {
            return _context.Database.SqlQuery<TEntity>(sql, parameters).AsQueryable();
        }
        ...
        {
            TEntity result = _context.Set<TEntity>().Add(entity);
        ...
        {
            IEnumerable<TEntity> result = _context.Set<TEntity>().AddRange(entites);
        ...
        public override IList<TEntity> Update(IEnumerable<TEntity> entites)
        {
            ...
            try
            {
                foreach (TEntity entity in entites)
                {
                    ...
                    if (dbEntityEntry.State == EntityState.Detached)
                    {
                        result.Add(_context.Set<TEntity>().Attach(entity));
                    }
                    dbEntityEntry.State = EntityState.Modified;
                }
            }
        ...
        public override IList<TEntity> Delete(IList<TEntity> entites)
        {
            ...
            foreach (TEntity entity in entites)
            {
                ...
                {
                    _context.Set<TEntity>().Attach(entity);
                    result.Add(_context.Set<TEntity>().Remove(entity));
                }
            }
        ...
        private void RefreshAll()
        {
            ...
            {
                foreach (DbEntityEntry entityTracker in _context.ChangeTracker.Entries())
                {
